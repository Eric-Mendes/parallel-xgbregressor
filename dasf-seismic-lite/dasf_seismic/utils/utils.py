...
def create_array(darray, kernel=None, hw=None, boundary="reflect", preview=None):
    """
    Description
    -----------
    ...
    if not is_dask_array(darray):
        chunk_size = compute_chunk_size(
            darray.shape, darray.dtype.itemsize, kernel=kernel, preview=preview
        )
...
def extract_patches(in_data, kernel, xp):
    ...
    patches = xp.lib.stride_tricks.as_strided(in_data, shape=shape, strides=strides)
...
def dask_cupy_angle_wrapper(data):
    return data.map_blocks(
        cp.angle, dtype=data.dtype, meta=cp.array((), dtype=data.dtype)
    )
