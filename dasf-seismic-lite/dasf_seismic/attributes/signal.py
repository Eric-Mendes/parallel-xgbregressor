...
from dasf_seismic.utils.utils import (create_array, extract_patches,
                                      set_time_chunk_overlap, trim_dask_array)
...
class FirstDerivative(Transform):
    ...
    def __lazy_transform(self, X, xndi, xp):
        ...
        result1 = result0.map_blocks(
            xndi.correlate1d,
            weights=xp.array([0.178947, 0.642105, 0.178947]),
            axis=axes[0],
            dtype=X.dtype,
            meta=xp.array((), dtype=X.dtype),
        )
    ...
    def __transform(self, X, xndi, xp):
        ...
        result0 = xndi.correlate1d(
            X, weights=xp.array([-0.5, 0.0, 0.5]), axis=self._axis
        )

        result1 = xndi.correlate1d(
            result0, weights=xp.array([0.178947, 0.642105, 0.178947]), axis=axes[0]
        )

        result2 = xndi.correlate1d(
            result1, weights=xp.array([0.178947, 0.642105, 0.178947]), axis=axes[1]
        )
...
class SecondDerivative(Transform):
    ...
    def __lazy_transform(self, X, xndi, xp):
        ...
        result0 = X.map_blocks(
            xndi.correlate1d,
            weights=xp.array([0.232905, 0.002668, -0.471147, 0.002668, 0.232905]),
            axis=self._axis,
            dtype=X.dtype,
            meta=xp.array((), dtype=X.dtype),
        )
        result1 = result0.map_blocks(
            xndi.correlate1d,
            weights=xp.array([0.030320, 0.249724, 0.439911, 0.249724, 0.030320]),
            axis=axes[0],
            dtype=X.dtype,
            meta=xp.array((), dtype=X.dtype),
        )
...
class HistogramEqualization(Transform):
    ...
    def _lazy_transform_gpu(self, X):
        ...
        hist, bins = da.histogram(
            X, bins=cp.linspace(da_min, da_max, 256, dtype=X.dtype)
        )
        ...
        return X.map_blocks(
            self.__interpolate, cdf=cdf, bins=bins, xp=cp, dtype=X.dtype
        )
    ...
    def _lazy_transform_cpu(self, X):
        ...
        hist, bins = da.histogram(
            X, bins=np.linspace(da_min, da_max, 256, dtype=X.dtype)
        )
        ...
        return X.map_blocks(
            self.__interpolate, cdf=cdf, bins=bins, xp=np, dtype=X.dtype
        )
    ...
    def _transform_gpu(self, X):
        ...
        hist, bins = cp.histogram(X, bins=cp.linspace(X_min, X_max, 256, dtype=X.dtype))
    ...
    def _transform_cpu(self, X):
        ...
        hist, bins = np.histogram(X, bins=np.linspace(X_min, X_max, 256, dtype=X.dtype))
...
class GradientMagnitude(Transform):
    ...
    def _lazy_transform(self, X, xndi):
        ...
        result = X.map_blocks(
            xndi.gaussian_gradient_magnitude, sigma=self._sigmas, dtype=X.dtype
        )
...
class PhaseRotation(Transform):
    ...
    def _lazy_transform(self, X, xsignal, xp):
        ...
        analytical_trace = X.map_blocks(
            xsignal.hilbert, dtype=X.dtype, meta=xp.array((), dtype=X.dtype)
        )

        result = analytical_trace.real * da.cos(phi) - analytical_trace.imag * da.sin(
            phi
        )
    ...
    def _transform(self, X, xsignal, xp):
        ...
        result = analytical_trace.real * xp.cos(phi) - analytical_trace.imag * xp.sin(
            phi
        )
