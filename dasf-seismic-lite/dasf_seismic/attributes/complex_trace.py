...
from dasf_seismic.utils.utils import (create_array, dask_cupy_angle_wrapper,
                                      local_events, set_time_chunk_overlap,
                                      trim_dask_array)
...
class DominantFrequency(Transform):
    def __init__(self, sample_rate=4, preview=None):
        ...
        self.__inst_freq = InstantaneousFrequency(
            sample_rate=sample_rate, preview=preview
        )
...
class ResponsePhase(Transform):
    ...
    def _lazy_transform_gpu(self, X):
        ...
        troughs = env.map_blocks(
            local_events, comparator=cp.less, is_cupy=True, dtype=X.dtype
        )

        troughs = troughs.cumsum(axis=-1)
        result = da.map_blocks(self.__operation, env, phase, troughs, cp, dtype=X.dtype)
    ...
    def _lazy_transform_cpu(self, X):
        ...
        troughs = env.map_blocks(
            local_events, comparator=np.less, is_cupy=False, dtype=X.dtype
        )

        troughs = troughs.cumsum(axis=-1)
        result = da.map_blocks(self.__operation, env, phase, troughs, np, dtype=X.dtype)
        result[da.isnan(result)] = 0
...
class ResponseFrequency(Transform):
    ...
    def _lazy_transform_gpu(self, X):
        ...
        result = da.map_blocks(
            self.__operation, env, inst_freq, troughs, cp, dtype=X.dtype
        )
        result[da.isnan(result)] = 0
    ...
    def _lazy_transform_cpu(self, X):
        ...
        troughs = troughs.cumsum(axis=-1)
        result = da.map_blocks(
            self.__operation, env, inst_freq, troughs, np, dtype=X.dtype
        )
        result[da.isnan(result)] = 0
...
class ResponseAmplitude(Transform):
    ...
    def _lazy_transform_gpu(self, X):
        ...
        result = da.map_blocks(self.__operation, env, X, troughs, cp, dtype=X.dtype)
    ...
    def _lazy_transform_cpu(self, X):
        ...
        result = da.map_blocks(self.__operation, env, X, troughs, np, dtype=X.dtype)
    ...
    def __operation(self, chunk1, chunk2, chunk3, xp):
        ...
        for i, j in np.ndindex(out.shape[:-1]):
            ...
            for ii in ints:
                idx = xp.where(chunk3[i, j, :] == ii)[0]
                peak = idx[chunk1[i, j, idx].argmax()]
                out[i, j, idx] = chunk1[i, j, peak] * xp.sign(chunk2[i, j, peak])
